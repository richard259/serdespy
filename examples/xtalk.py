...
import skrf as rf
import scipy as sp
#load in touchstone files

s4p_dir = "C:/Users/richa/Desktop/Thesis/mellitz_3ck_04_1119_CACR/"
...
#%%
xt_response = np.zeros(64*10000)

for i in range(h_xtalk.shape[0]):
    xt_signal = sdp.Transmitter(data[(i+1)*10000:(i+2)*10000], voltage_levels, 26.56e9)
    xt_signal.oversample(samples_per_symbol)
    xt_response = xt_response + sp.signal.fftconvolve(xt_signal.signal_ideal, h_xtalk[i][:], mode = "same")
